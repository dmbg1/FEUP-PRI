...
def metrics(query_num):
    # Relevant Total
    all_relevant = list(map(lambda el: el.strip(), open("./M2/queries/q" + str(query_num) + "_rels.txt").readlines()))
    #print(relevant)
    ...
    for index in range(len(all_results)):
        ...
        if(index == 0):
            with open('./M2/results/results_no_schema_q' + str(query_num) + '.tex','w') as tf:
                tf.write(df.to_latex())
        elif(index == 1):
            with open('./M2/results/results_no_weights_q' + str(query_num) + '.tex','w') as tf:
                tf.write(df.to_latex())
        elif(index == 2):
            with open('./M2/results/results_normal_q' + str(query_num) + '.tex','w') as tf:
                tf.write(df.to_latex())
    ...
    plt.savefig('./M2/results/precision_recall_q' + str(query_num) + '.pdf')
